...
export const FinderWindow = ({ window, manifest, isActive, onClose, onMinimize, onMaximize, onFocus, onDrag, isMobile }: FinderWindowProps) => {
  ...
  const renderContent = () => {
    switch (window.content) {
      case 'about':
        return (
          <div className="p-6 md:p-8 text-center bg-gradient-to-b from-gray-50 to-gray-100 h-full overflow-auto">
            <div className="flex justify-center mb-6">
              {manifest.personalInfo.avatar ? (
                <img src={manifest.personalInfo.avatar} alt="" className="w-20 h-20 md:w-24 md:h-24 rounded-full object-cover shadow-xl ring-4 ring-white" />
              ) : (
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-2xl md:text-3xl font-semibold shadow-xl ring-4 ring-white">
                  {manifest.personalInfo.name[0]}
                </div>
              )}
            </div>
            <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-1">{manifest.personalInfo.name}</h2>
            <p className="text-gray-500 mb-6 text-base md:text-lg">{manifest.personalInfo.title}</p>
            
            <div className="flex justify-center gap-6 mb-6">
              <div className="text-center">
                <div className="text-xl md:text-2xl font-bold text-gray-800">{manifest.projects.length}</div>
                <div className="text-xs text-gray-500 uppercase tracking-wider">Projects</div>
              </div>
              <div className="w-px bg-gray-300" />
              <div className="text-center">
                <div className="text-xl md:text-2xl font-bold text-gray-800">{manifest.experience.length}</div>
                <div className="text-xs text-gray-500 uppercase tracking-wider">Roles</div>
              </div>
              <div className="w-px bg-gray-300" />
              <div className="text-center">
                <div className="text-xl md:text-2xl font-bold text-gray-800">{manifest.skills.reduce((a, s) => a + s.items.length, 0)}</div>
        ...
        if (isMobile && selectedProject) {
          return (
            <div className="h-full overflow-auto bg-gray-50">
              <div className="sticky top-0 bg-gray-50 border-b border-gray-200 px-4 py-2.5 z-10">
                <button onClick={() => setSelectedProject(null)} className="flex items-center gap-1 text-blue-500 text-sm font-medium">
              ...
              <div className="p-5">
                <div className="flex items-start gap-4 mb-5">
                  <div className="w-16 h-16 bg-gradient-to-br from-blue-400 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0">
                ...
                <p className="text-sm text-gray-700 leading-relaxed mb-5 break-words" style={{ wordBreak: 'break-word', overflowWrap: 'break-word' }}>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedProject.technologies.map((tech, i) => (
                        <span key={i} className="text-[11px] bg-white text-gray-700 px-2.5 py-1 rounded-full border border-gray-200 shadow-sm">{tech}</span>
                      ))}
                    </div>
                  </div>
                )}
          ...
          <div className="flex h-full">
            ...
            <div className="flex-1 flex flex-col bg-white">
              ...
              <div className="flex-1 flex overflow-hidden">
                <div className={`${!isMobile && selectedProject ? 'w-1/2 border-r border-gray-200' : 'flex-1'} p-4 overflow-auto transition-all`}>
                  <div className={`grid ${isMobile ? 'grid-cols-3' : selectedProject ? 'grid-cols-2' : 'grid-cols-4'} gap-2`}>
                      ...
                      >
                        <div className="p-6">
                          ...
                          <div className="mb-6">
                            ...
                            <p className="text-sm text-gray-700 leading-relaxed break-words" style={{ wordBreak: 'break-word', overflowWrap: 'break-word' }}>{selectedProject.description}</p>
                          </div>
                          {selectedProject.technologies && selectedProject.technologies.length > 0 && (
                            <div className="mb-6">
                              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Technologies</h4>
                  ...
                  <div key={idx} className="relative pl-6 pb-4 border-l-2 border-blue-200 last:border-transparent">
                    ...
                    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition">
                      <div className={`flex ${isMobile ? 'flex-col gap-1' : 'justify-between items-start'} mb-2`}>
                        ...
                        <span className={`text-xs text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full font-medium ${isMobile ? 'self-start' : ''}`}>
                        ...
                        <ul className="mt-3 space-y-1.5">
                          {exp.achievements.map((achievement, i) => (
                            <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
                              ...
                              <span className="break-words" style={{ wordBreak: 'break-word', overflowWrap: 'break-word' }}>{achievement}</span>
                            </li>
                          ))}
                        </ul>
                      )}
                      {exp.technologies && exp.technologies.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-3">
                          {exp.technologies.slice(0, isMobile ? 3 : 5).map((tech, i) => (
        ...
        return (
          <div className="flex h-full bg-gray-100">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              {isMobile && (
                <div className="flex overflow-x-auto gap-1.5 p-2 bg-gray-50 border-b border-gray-200 flex-shrink-0">
                  {manifest.skills.map((group, idx) => (
                    <button
                      key={idx}
                      onClick={() => setSelectedSkillCategory(idx)}
              ...
              <div className="flex-1 p-4 md:p-6 overflow-auto">
                <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-2">{selectedGroup.category}</h2>
                <p className="text-gray-500 text-sm mb-4 md:mb-6">{selectedGroup.items.length} skills in this category</p>
      ...
      case 'education':
        return (
          <div className={`flex ${isMobile ? 'flex-col' : ''} h-full bg-gray-900`}>
            {!isMobile && (
            ...
            <div className={`flex-1 flex items-center justify-center ${isMobile ? 'p-4' : 'p-8'} overflow-auto`}>
              <div className={`${isMobile ? 'w-full' : 'max-w-lg w-full'}`}>
                {manifest.education.map((edu, idx) => (
                  <div key={idx} className={`bg-white rounded-xl shadow-2xl ${isMobile ? 'p-5' : 'p-8'} mb-4 md:mb-6`}>
                    <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                      <div className={`${isMobile ? 'w-12 h-12' : 'w-16 h-16'} bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg`}>
                        <span className={`${isMobile ? 'text-2xl' : 'text-3xl'}`}>🎓</span>
                      </div>
                      <div>
                        <h3 className={`${isMobile ? 'text-base' : 'text-xl'} font-bold text-gray-900`}>{edu.degree}</h3>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-gray-700 font-medium text-sm">{edu.institution}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <svg className="w-5 h-5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                      ...
                      {edu.gpa && (
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-gray-600 text-sm">GPA: <strong className="text-gray-800">{edu.gpa}</strong></span>
                        </div>
                      )}
                    </div>
                  </div>
  ...
  if (isMobile) {
    return (
      <motion.div
        ...
        transition={{ type: 'spring', stiffness: 400, damping: 30 }}
        onMouseDown={onFocus}
        className="fixed left-0 right-0 top-[26px] bottom-[72px] flex flex-col overflow-hidden"
        style={{ zIndex: window.zIndex }}
      >
        <div className="h-11 flex items-center px-3 gap-2 flex-shrink-0 border-b bg-gradient-to-b from-gray-100 to-gray-200 border-gray-300">
          <button 
            onClick={onClose}
            className="w-7 h-7 rounded-full bg-gray-200 hover:bg-gray-300 active:bg-gray-400 flex items-center justify-center transition"
          >
            <svg className="w-4 h-4 text-gray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/>
            </svg>
          </button>
          <span className="flex-1 text-center text-[13px] text-gray-700 font-medium truncate">{window.title}</span>
          <div className="w-7" />
        </div>
